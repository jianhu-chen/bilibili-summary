...
- Fetches CC subtitles when available (faster, free)
- Falls back to ASR transcription via OpenAI-compatible Whisper API (parallel processing for long videos)
...
4. **Segment** — Splits audio into chunks if > 50 min, compresses to 32k mono
5. **Transcribe** — Calls Whisper ASR API in parallel (max 4 concurrent)
