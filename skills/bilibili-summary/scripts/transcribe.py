#!/usr/bin/env python3
"""
Bilibili video transcription tool with subtitle-first strategy.

Attempts to fetch CC subtitles first; falls back to ASR (Whisper API) only
when subtitles are unavailable.

Multi-step workflow to avoid Bash tool timeout:
  Step 1 - prepare: Check deps, try subtitle extraction, fallback to audio
                      download + split + compress + validate sizes
  Step 2 - transcribe-all: Transcribe all chunks in parallel (ASR fallback)
  Step 3 - collect: Merge transcripts (from subtitles or ASR)
  Step 4 - cleanup: Remove workdir

Usage:
  transcribe.py prepare <bilibili_url> <workdir>
...
Exit codes:
  ...
  13 = curl not found
  14 = Bilibili anti-scraping block (HTTP 412)
  15 = BILIBILI_COOKIES not set
"""
...
def write_cookie_file(workdir):
    """Read BILIBILI_COOKIES env var (cookie text content) and write to a temp file.

    Returns the cookie file path. Exits with code 15 if not set.
    """
    content = os.environ.get("BILIBILI_COOKIES", "").strip()
    if not content:
        eprint("[Error] Environment variable BILIBILI_COOKIES is not set")
        eprint("[Error] Cookies are required to access Bilibili videos")
        eprint("[Error] Please set BILIBILI_COOKIES with your cookie content (Netscape format)")
        sys.exit(15)
    cookie_path = os.path.join(workdir, "cookies.txt")
    with open(cookie_path, "w", encoding="utf-8") as f:
        f.write(content)
    os.chmod(cookie_path, 0o600)
    eprint(f"[Cookie] Written cookie file: {cookie_path}")
    return cookie_path


def _check_antiscraping(result):
    """Check if yt-dlp returned HTTP 412 (Bilibili anti-scraping). Exit 14 if so."""
    combined = (result.stderr or "") + (result.stdout or "")
    if "412" in combined or "Precondition Failed" in combined:
        eprint("[Error] Bilibili returned HTTP 412 (anti-scraping block)")
        eprint("[Error] Your cookie may have expired, please update BILIBILI_COOKIES with fresh cookie content")
        sys.exit(14)
...
def format_metadata(meta):
    ...
    return {
        ...
        "coin_count": meta.get("coin_count"),
        "favorite_count": meta.get("favorite_count"),
        "danmaku_count": meta.get("danmaku_count"),
...
def parse_srt_to_text(srt_path):
    ...
    for block in blocks:
        ...
        # SRT format: line 0 = sequence number, line 1 = timestamp, line 2+ = text
        if len(lines) >= 3:
            text_lines = lines[2:]
...
def cmd_prepare(url, workdir, lang_hint=None):
    ...
    if not is_bilibili_url(url):
        eprint(f"[Error] Not a Bilibili URL: {url}")
        eprint("[Error] Supported formats: https://www.bilibili.com/video/BV..., https://www.bilibili.com/video/av..., https://b23.tv/...")
        sys.exit(2)
    ...
    cookie_file = write_cookie_file(workdir)
    ...
    eprint(f"[Info] Uploader: {meta['uploader']}")
    ...
    # Try fetching CC subtitles first
    subtitle_found = False
    t_subs = time.time()
    eprint("[Prepare] Step 2/3: Trying to fetch CC subtitles...")
    ...
    if srt_files:
        ...
        try:
            ...
            if len(transcript.strip()) < 50:
                eprint(f"[Warn] Subtitle content too short ({len(transcript)} chars), likely empty or unusable")
                eprint("[Warn] Falling back to ASR...")
            else:
                transcript_path = os.path.join(subs_dir, "transcript.txt")
                with open(transcript_path, "w", encoding="utf-8") as f:
                    f.write(transcript)
                subtitle_found = True
                eprint(f"[Prepare] Subtitle parsed: {len(transcript)} chars -> transcript.txt")
        except Exception as e:
            eprint(f"[Warn] Failed to parse subtitle file: {e}")
            eprint("[Warn] Falling back to ASR...")
    else:
        eprint("[Prepare] No CC subtitles available")

    eprint(f"[Time] Subtitle check: {time.time() - t_subs:.1f}s")

    chunk_count = 0

    if not subtitle_found:
        # Fallback: ASR pipeline
        eprint("[Prepare] Step 3/3: Falling back to ASR pipeline...")

        # Download audio
        t_dl = time.time()
        eprint("[Prepare] Downloading audio...")
        ...
        t_split = time.time()
        eprint("[Prepare] Preparing audio files...")
        result = run_cmd([
            "ffprobe", "-v", "quiet",
        ...
        eprint("[Prepare] Step 3/3: Skipped (using CC subtitles)")
...
def cmd_collect(workdir):
    ...
    transcript_path = os.path.join(workdir, "subs", "transcript.txt")
    if os.path.exists(transcript_path):
        eprint("[Collect] Using CC subtitle transcript")
        with open(transcript_path, "r", encoding="utf-8") as f:
            text = f.read().strip()
        print(text)
        eprint(f"[Done] Subtitle transcript: {len(text)} chars")
        eprint(f"[Time] collect total: {time.time() - t_start:.1f}s")
        return

    # Fallback: merge ASR chunk transcripts
    chunks_dir = os.path.join(workdir, "chunks")
...
def cmd_cleanup(workdir):
    t_start = time.time()
    if not workdir.startswith("/tmp/bili-summary-"):
        eprint(f"[Error] Refusing to delete unsafe path: {workdir} (only /tmp/bili-summary-* allowed)")
        sys.exit(2)
...
def main():
    ...
    if command == "--version":
        print(f"bilibili-summary skill v{__version__}")
        sys.exit(0)
