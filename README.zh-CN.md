...
- [x] 优先提取 CC 字幕（更快、免费）
- [x] 无字幕时回退到 ASR 语音转文字（OpenAI 兼容 Whisper API，长视频自动并行处理）
- [x] AI 生成包含要点摘要和视频详情的 Markdown 报告
- [x] 超过 50 分钟的视频自动分段处理
...
4. ✂️ **分段** — 超过 50 分钟自动切分，所有分片统一压缩至 32k mono
5. 🎙️ **转录** — 并行调用 Whisper ASR API（最大并发 4）
